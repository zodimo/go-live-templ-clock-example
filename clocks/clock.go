...
func clockRender(w io.Writer, c *page.Component) error {
	clockState, ok := c.State.(*ClockState)
	if !ok {
		return fmt.Errorf("no clock data")
	}
	return ClockView(clockState).Render(context.Background(), w)
}

// NewClock create a new clock component.
func NewClockComponent(ID string, h *live.Handler, s *live.Socket, timezone string) (*page.Component, error) {
